#!/usr/bin/env python3
"""Exercise combo4, splash, knockback and lifecycle cleanup with two clients."""
...
from test_combos import trusted_definition_hash
...
def schema_text(game_server: str, database: str) -> str:
    ...
    try:
        ...
        raise RuntimeError(
            "The selected game route does not expose the finisher database schema "
...
def main() -> None:
    ...
    parser.add_argument(
        "--report", type=Path, default=ROOT / ".local/p2-finisher/finisher-report.json"
    )
    options = parser.parse_args()
    if not options.database.startswith("mt2-p2-"):
        raise RuntimeError("Finisher smoke requires a disposable mt2-p2- database.")
    ...
    try:
        ...
        if any(name not in schema for name in required_schema):
            raise RuntimeError("The selected database does not expose the protocol 9 schema.")
        definition_hash = trusted_definition_hash()
        with tempfile.TemporaryDirectory(prefix="finisher-", dir=ROOT / ".local") as scratch:
            stage = Path(scratch)
            stage_project(stage)
            scripts = (
                Path("tests/combo_smoke.gd"),
                Path("tests/root_motion_smoke.gd"),
                Path("tests/finisher_smoke.gd"),
                Path("tests/finisher_lifecycle_smoke.gd"),
            )
            ...
            for script, label in (
                ...
                ("tests/combo_smoke.gd", "combo-smoke"),
                ("tests/root_motion_smoke.gd", "root-motion-smoke"),
                ("tests/finisher_smoke.gd", "finisher-smoke"),
                ("tests/finisher_lifecycle_smoke.gd", "finisher-lifecycle-smoke"),
            ):
                run_godot(
                    options.godot,
                    stage,
            ...
            tokens = [auth.create_account(f"finisher_{suffix}_{index}") for index in range(2)]
            fixture = stage / "finisher-config.json"
            private_report = stage / "finisher-report.json"
            ...
            try:
                output = run_godot(
                    ...
                    "--script",
                    "res://tests/finisher_lifecycle_smoke.gd",
                    "--",
                    "--account-config",
                    str(fixture),
                    timeout=300,
            ...
            if not private_report.is_file():
                raise RuntimeError("Godot did not produce its finisher report.")
            report = json.loads(report_path.read_text())
            if not report.get("passed") or "MT2_MULTIPLAYER_SMOKE PASS" not in output:
                raise RuntimeError(f"Godot finisher checks failed; report: {report_path}")
            print(f"Verified {len(report['checks'])} finisher checks; report: {report_path}")
    finally:
        if not auth.logout():
