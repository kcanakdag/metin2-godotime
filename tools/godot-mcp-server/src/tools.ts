  ...
  for (const def of TOOL_DEFINITIONS) {
    ...
    server.tool(def.name, def.description, schema, async (args) => {
      try {
        ...
        // Return screenshots as MCP images, so clients can see them directly.
        if (result && typeof result === "object" && "base64" in result && typeof result.base64 === "string" && result.base64) {
