...
def main() -> None:
    ...
    if args.mob_catalog is not None:
        catalog = json.loads(args.mob_catalog.read_text())
        if not (isinstance(catalog, dict) and isinstance(catalog.get("mobs"), list)):
            parser.error(
                f"--mob-catalog needs a compiled gameplay catalog with a 'mobs' list, "
                f"not {args.mob_catalog} (a regeneration profile has 'entries')"
            )
