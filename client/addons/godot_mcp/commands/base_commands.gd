...
func _queue_input(events: Array) -> void:
	# Autoloads exist in the game process, not in the editor's SceneTree.
	# Publish separate complete batches so rapid press/release calls cannot race.
	var directory := OS.get_user_data_dir().path_join("mcp_input_batches")
	DirAccess.make_dir_recursive_absolute(directory)
	var path := directory.path_join("%020d.json" % Time.get_ticks_usec())
