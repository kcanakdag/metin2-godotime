...
func _json_value(value: Variant, depth: int = 0) -> Variant:
	# Keep structured observations intact without serializing engine objects.
	if depth >= 16:
		return "<maximum inspection depth>"
	match typeof(value):
