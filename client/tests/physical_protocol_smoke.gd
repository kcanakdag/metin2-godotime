extends SceneTree
## Isolated protocol-27 gate check; no generated schema or server is required.
...
func _run() -> void:
	_check(GameConnection.EXPECTED_PROTOCOL_VERSION == 27, "client expects protocol 27")
	var legacy := GameConnection.new()
	legacy._session = 1
	legacy.world_info = {"protocol_version": 26}
	legacy._on_subscription_applied(1)
	_check(legacy.state == "error", "protocol 26 is rejected before joining")
	_check(
		legacy.state_message.contains("protocol mismatch"),
		"protocol-26 rejection explains the incompatible client/server contract"
	)
	legacy.queue_free()

	var current := GameConnection.new()
	current._session = 1
	current.world_info = {"protocol_version": 27}
	current._on_subscription_applied(1)
	_check(current.state != "error", "protocol 27 clears the protocol gate")
	current.monsters = [
